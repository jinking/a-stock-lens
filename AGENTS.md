...
- 不提交未在设计文档中确认的默认阈值。
- **文档与提交信息一律使用中文**（项目所有者的明确要求，2026-09-17）：
  - 新增或修改的 `docs/`、`README.md`、`AGENTS.md`、脚本注释、`configs/` 内注释都写中文；
  - `git commit -m` 的主题与正文都用中文，不再使用英文 conventional commit 前缀；
  - 代码标识符、类型名、既有英文 API 名称保持原样；引用设计文档时保留原文标题。
- 每次切片的结论、裁决与新约定写入 `.workbuddy/memory/<YYYY-MM-DD>.md`（中文），供后续会话继承。
