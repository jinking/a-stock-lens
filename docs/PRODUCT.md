...
Trade Gate 是可选的下游人工交易准入纪律工具，不替代 Candidate/Watchlist，也不自动下单。`PASS` 表示该次交易意图满足准入条件（eligible），不是买入推荐；`WAIT` 表示条件未完成；`NO_TRADE` 表示不应按正常路径继续。所有时点事实与审计结果通过独立 Ledger 留痕。
...
- AkShare；
- 腾讯 WeStock CLI（财务三大表；见设计规格 §24 补遗，2026-09-16）；
- 交易所/公开数据；
- 可替换免费备用源。
