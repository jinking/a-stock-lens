...
> **实测更正注解（2026-09-20，所有者批准加注；本节原文保留不动）**
>
> 本节「架构只能读评分因子、从而**迫使**实现者替换业务指标」的因果叙事**与实测不符**，后续不得作为已证事实引用。
>
> 实测（主理人独立复核，`data/snapshots/STRATEGY/2026-09-19.json`）：全部 **13,818** 条记录（6 策略 × 2,303 只）
> 的 `factor_snapshot` 长度均为 **24**，且均含 `roe_ttm`（键名为 `factor`）；growth 记录中缺 `roe_ttm` 的条数为 **0 / 2,303**。
> 即旧接口下 `roe_ttm` 本就取得到，旧缺陷（Growth 以 `net_profit_parent_cagr_3y` 替换 `roe_ttm`）的
> 直接成因更可能是人工替换，而非「被架构逼迫」。
>
> 本节的结论——**必须分离策略评分证据与资格证据**——仍然成立，并已按 §4 以独立
> `QualificationContext.factors` 落地（概念分离更显式、仍然正确）；仅止于更正上述因果叙事。
