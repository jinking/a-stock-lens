...
- **`neodata` 是一等 Provider（2026-09-17 修订，见 §24.1）。** 更新前的判断是"只作
  candidate 级、只经 Adapter 使用"；当日实测推翻了其中两条假设（见下），因此改为：
  它是**估值、行业与语义维度三类主数据的来源**，同时是财务数据的交叉验证源。
- AkShare 保留它已有的两个角色：交易所名单（标的枚举）与日线行情。

§3、§4 与 §5.2 的其余内容保持不变。

### 24.1 `neodata` 的实测结论与定位（2026-09-17）

推翻的两条旧假设：

1. **"只能逐标的"不成立。** 实测一次查询可带 10 只标的（3.1 秒返回），批量可行；
2. **"Markdown 形态不适合入库"不成立。** 本仓库已为 WeStock 实现了 Markdown 表格解析，
   形态本身不是障碍。

成立且必须写进设计的限制：

1. **意图匹配决定能否取到数据。** 服务端对未命中查询返回 `1001 未命中意图`，
   因此查询措辞固化为 Provider 内的模板并由测试钉住，调用方不能自由拼自然语言；
2. **批量回答是部分的，`apiData.entity` 不能作为覆盖依据。** 实测请求 3 只标的时：
   利润表只回 2 只、估值只回 1 只，而 `entity` 三只都列了。覆盖必须按内容判定，
   并对缺口补抓一次；
3. **不做标的枚举。** 名单仍由 AkShare 提供，本 Provider 只按给定名单取数。

因此定位是：

| 数据维度 | 结论 |
| --- | --- |
| 估值（PE/PB、历史分位、行业相对标签、逐日 PS/市现率/股息率/EV/PEG） | **主数据源**：唯一同时给出历史分位与行业相对标签的来源 |
| 行业/板块（板块估值 + 成分股明细含 PE TTM/总市值/资金流） | **主数据源**：Industry Trend 的行业侧输入，也是全市场估值的批量路径（按板块迭代，而非 5,500 次单标的查询） |
| 语义维度（主营构成、供应链、业绩会纪要） | **唯一来源**：WeStock 的 `profile` 只有一句业务描述文本 |
| 财务三大表 | **交叉验证源**：与 WeStock 数值一致（实测茅台 H1 完全相同），不一致即信号 |
| 全市场标的名单、日线 | 不使用：由 AkShare 提供 |

凭证：12 小时有效，v1.6.0 起缓存在插件目录内（`plugins/cache/*/finance-data/*/skills/.neodata_token`），
旧路径 `~/.workbuddy/.neodata_token` 是迁移前的位置；刷新由平台侧完成，
`astock doctor` 报告状态与刷新方式。
