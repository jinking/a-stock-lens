...
**实测回填（2026-09-26，Task 13）：** 两个中间检查点已按实测收口：批次 2 末 **1077**（Task 9 零偏差）、批次 1–4 末 **1063**（Task 10 −8、Task 11 −6 精确达成）；与计划文字（1077 / 1051）的差来自 Task 5 单表 −14 与 Task 10/11 实际减量，方向与原因见 §5.4 与各任务行。Task 15 缺口 = 1063 − 1040 = **23**。
...
| Task 15 | `37644c1` + `7dacdd2` + `fb20597` + `7432669` + `4745bab` + `b941560` + `9a63903` + `b94eb8a` | −25 | 1038 | 1063 − 25 = 1038 ≤ 1040 ✓（缺口 23 补齐，余量 2） | 备用池 case 级收口。**范围**：规格 §5 备用池 = 基线 35 个「6–9 例」文件（9 例 ×8、8 例 ×10、7 例 ×8、6 例 ×9，合计 262 例，Task 12 已并入域文件）；其中 14 文件（或其已审计部分，含冻结 `test_extension_contracts.py`，一行未动）已由 Task 4/6/7/9/10 做过 case 级审计、**不重复计减**，Task 15 审计其余 **21 个池文件的现行落点**（26 个家族行）+ **2 个已登记软候选**。**判定分布**：§10 共 28 行 = 13 行 MERGE（11 个池家族 + 2 个软候选）+ 15 行 KEEP。**实测减量 −25** = 软候选 −2（`37644c1`）＋交易日历 −3（`7dacdd2`）＋artifacts 校验器 −7（`fb20597`）＋动量扫描器 −1（`7432669`）＋CLI 表层 −4（`4745bab`）＋CLI 集成入口 −6（`b941560`）＋候选阶段/政策 −2（`9a63903`+`b94eb8a`）；全部并表满足 §4.1 三条（遍历全部成员 / 先收集再断言 / 失败消息点名 label），被吸收成员的每条断言逐行落为表内期望列或行内保留，`==`/`is`/`in` 原语义未放宽，无新增断言、无 `skip`/`xfail`/`importorskip`。**缺口已补齐（1038 ≤ 1040，余量 2）→ 不触发规格 §5 目标复议**。全量 1038 passed 全绿 + 收口 collect 1038；`ruff check tests/` 绿、8 个改动文件 `ruff format --check` 绿、`mypy` 167 源码文件零问题；`src/`、`configs/` 零 diff；冻结 5 文件零改动 |
...
**范围**：规格 §5 的备用池 = 基线 35 个「6–9 例」文件（9 例 ×8、8 例 ×10、7 例 ×8、6 例 ×9，合计 262 例），已由 Task 12 整体并入域文件；本节按 case 级审计其**现行落点**（以域文件内 `# 来源：` 分节为界，不按基线文件行号）。

**不重复计减的 14 个池文件**（已由前序任务 case 级审计，减量已计入各自任务行；其中 5 个文件的**余量**仍列入本表）：`test_signal_detector.py`（Task 6，8→1）、`test_strategy_parity.py`（Task 4，7→2）、`test_qualification_production_rules.py`（Task 4，8→3）、`test_dividend_yield_factor.py`（Task 7，7→2）、`test_candidate_evidence.py`（Task 9，8→6）、`test_universe_config.py`（Task 9，7→5）、`test_market_evidence.py`（Task 9，7→6）、`test_command_snapshot_ownership.py`（Task 9，6→4）、`test_cli_industry_loader.py` / `test_candidate_calibration_cli.py`（Task 10 T11 裁决「两支都 KEEP」，未删例）、`test_candidate_qualification_pipeline.py`（Task 10 T5，6→5）、`test_qualified_discovery.py`（Task 10 T8 裁决 KEEP）、`test_candidate_builder.py`（Task 10 T3 裁决 KEEP）、`test_extension_contracts.py`（**冻结，一行未动**）。

Task 15 审计其余 **21 个池文件的现行落点**（26 个家族行）+ **2 个已登记软候选**，共 28 行判定如下。

**审计判定**（逐家族；「减量」为本任务对该家族的实测净减）：

| 现行文件 | 家族（原用例） | 判定 | 减量 | 理由 |
| --- | --- | --- | ---: | --- |
| `tests/unit/test_platform_basics.py` | `test_trading_calendar.py`（池 6 例） | MERGE → `test_is_trade_date_matches_every_documented_day`（6 行）+ `test_latest_trade_date_matches_every_documented_day`（3 行） | −3 | 3 条逐日判定（工作日 / 周末 / 法定假日）与 2 条「最近交易日」各是同一断言的参数枚举，比对沿用原 `is` / `==` 语义；`test_custom_dates_override` 走自定义日期的另一构造分支，KEEP |
| `tests/artifacts/test_candidate_semantic_validator.py` | 整文件即池内（池 9 例；§3.2：可表化、断言不可删） | MERGE → `test_every_bad_candidate_record_hits_its_expected_check`（8 行） | −7 | 8 条「坏记录必须命中指定 check」是同一断言的参数枚举（label / overrides / expected_check / observed 片段），缺版本行的原 observed 断言逐字携带；valid-passes 例保留独立；断言零删除 |
| `tests/unit/test_strategy_long_tail.py` | `test_momentum_scanner.py`（池 9 例） | MERGE → `test_missing_or_null_required_factor_makes_the_symbol_ineligible`（2 行） | −1 | 「缺因子 / NULL 因子」两条不合格判定是同一断言的参数枚举（沿用原 `is not False` 与逐理由 `in` 语义）；其余 7 条观察点不同，KEEP |
| `tests/unit/test_cli_surface.py` | `test_cli.py`（池 9 例） | MERGE → `test_startup_commands_report_their_surface`（2 行）+ `test_preview_commands_report_their_verdicts_and_write_no_snapshot`（2 行） | −2 | 两组各为同一断言的参数枚举（--help / doctor 逐片段 `in` stdout；scan / universe build 退出码、片段、不写 json 逐行点名）；其余 5 条命令行为各异，KEEP |
| `tests/unit/test_cli_surface.py` | `test_cli_industry_loader.py`（池 7 例） | MERGE → `test_industry_file_selection_and_mapping`（2 行）+ `test_non_eligible_files_are_ignored`（2 行） | −2 | 两组各为同一断言的参数枚举（精确日期 / 最近先前日期；未来日期 / 非 ISO 文件名），文件名与映射逐行 `==`；`step4_no_data`、`ambiguous_membership`（Task 10 T11 KEEP）、`includes_supplemental` 三条语义不同，KEEP |
| `tests/unit/test_cli_surface.py` | `test_cli_research_adapter.py`（池 9 例） | KEEP | 0 | 9 条各守适配器协议的一个面（提交 / 透传 / 状态 / 结果 / 未配置 / 失败 / 坏输出 / 缺字段），断言结构各异，无同一断言的参数枚举 |
| `tests/integration/test_cli_entry_flows.py` | `test_calibration_readiness_cli.py`（池 9 例） | MERGE → `test_unusable_mapping_date_declarations_are_refused`（2 行） | −1 | 「不可用的映射日期声明必须以 Click 用法错误拒绝」同一断言的参数枚举（无时区 / 无外部映射）；其余 7 条观察点不同，KEEP |
| `tests/integration/test_cli_entry_flows.py` | `test_candidate_calibration_cli.py`（池 6 例） | MERGE → `test_calibrate_candidates_fails_loudly_on_bad_industry_maps`（3 行） | −2 | 「坏行业映射必须响亮失败」同一断言的参数枚举（缺文件 / 重复 symbol / 缺表头）；「重复 symbol」行即 Task 10 T11 裁决保留的那一支，断言逐条落入行内期望列 |
| `tests/integration/test_cli_entry_flows.py` | `test_screen_cli.py`（池 9 例） | MERGE → `test_screen_unserviceable_requests_fail_loudly`（2 行） | −1 | 「screen 无法诚实出表时必须响亮失败」同一断言的参数枚举（缺 STRATEGY 快照 / 快照无所请求策略）；其余 7 条输出语义各异，KEEP |
| `tests/integration/test_cli_entry_flows.py` | `test_qualified_cli.py`（池 9 例） | MERGE → `test_qualified_missing_snapshot_fails_loudly`（2 行）+ `test_qualified_unhonorable_requests_fail_loudly`（2 行） | −2 | 两组各为同一断言的参数枚举（缺 FACTOR / 缺 STRATEGY 快照；非法资格配置 fail-closed / 未批准策略显式 not-found） |
| `tests/integration/test_cli_entry_flows.py` | `test_valuation_backfill_cli.py`（池 7 例） | KEEP | 0 | 7 条钉子各不相同（轮次合并只问缺口 / 缺口不闭合退出非零 / 已覆盖不再问 / 缺 universe 文件报错 / 非正批量拒绝 / coverage 分母 / 空 universe 拒绝），无同一断言的参数枚举 |
| `tests/unit/test_candidate_stage.py` | `test_candidate_builder.py`（池 8 例） | MERGE → `test_the_builder_leaves_next_action_at_the_inert_default`（2 行） | −1 | 「缺省即惰性 IGNORE、绝不自行推导 next_action」两条仅输入不同、断言结构完全一样（无策略结果 / 有 100 分结果）；Task 10 T3 的 E1 / F1 两条各钉一种决策→动作映射、不能并表；其余 5 条观察点各异，KEEP |
| `tests/unit/test_candidate_stage.py` | `test_candidate_policy.py`（池 6 例） | MERGE → `test_a_failed_policy_verdict_produces_no_candidate`（2 行） | −1 | 「policy 判定不通过就没有 Candidate」（`candidates == ()`）两条仅输入不同、断言完全一样（分数再高也覆盖不了拒绝一切的政策 / 结果为 ineligible）；protocol conformance、未配置政策报错、policy 决定候选生成 三条是不同上游状态与观察点，KEEP |
| `tests/unit/test_api.py` | 软候选 1（Task 6 登记：`test_universe_route_404s_when_no_universe_was_built`） | MERGE → `MISSING_SNAPSHOT_CASES` 第 7 行 | −1 | 与表内 `/stocks` 行同读 UNIVERSE 快照、同一个 `_read` 抛出点名日期的 404（404 + `DAY in detail`），两断言逐字落为行内期望列；`test_candidate_route_404s_when_nothing_was_scanned`（只断言 404）不属本池、未动 |
| `tests/unit/test_universe_builder.py` | 软候选 2（Task 7 登记：`test_a_symbol_that_breaks_two_rules_records_both`） | MERGE → `EXCLUSION_RULE_CASES` 第 7 行 | −1 | 「同一标的命中两条排除规则」的元组全等断言由 `expected_rules` 列逐字承接（已核对表内第 5 / 6 行同为 `000007.SZ` 且 `is not None` 分支可表达该断言） |
| `tests/integration/test_pipeline_flows.py` | `test_analysis_pipeline.py`（池 9 例） | KEEP | 0 | 9 条各钉一条业务步骤的独立观察点（因子 / 宇宙 / 策略结果、不写盘、同一业务步骤、排除规则逐条触发、延迟规则只报不施、手工算序、宇宙过滤、短史 NULL、缺输入不成 0），断言异构 |
...
| `tests/integration/test_pipeline_flows.py` | `test_valuation_pipeline.py`（池 6 例） | KEEP | 0 | 6 条各钉一个落盘 / PIT / 因子隔离 / 指标一致 / 幂等观察点，断言异构 |
...
| `tests/unit/test_quality_gates.py` | `test_daily_bar_quality_gate.py`（池 6 例） | KEEP | 0 | 6 个不同断言面（干净通过 / 规则逐条触发 / P2 不阻断 / 空数据 P1 / 只保留未标记 / 不改输入） |
| `tests/unit/test_quality_gates.py` | `test_financial_quality_gate.py`（池 8 例） | KEEP | 0 | 不同上游状态与观察点（缺失≠缺陷、不可读单元使键无效、重复键同删、跨期非重复、空报表 P1、单坏点不阻断全市场、findings 带键） |
| `tests/unit/test_universe_stage.py` | `test_discovery_service.py`（池 8 例） | KEEP | 0 | 8 个不同断言面（排序 / 平局序 / 缺失值序与保留 / 过滤器 / 查询校验 / 覆盖先于过滤 / 汇总 / 空汇总） |
| `tests/unit/test_universe_stage.py` | `test_qualified_discovery.py`（池 6 例） | KEEP | 0 | 6 个观察点（只返回双通过、确定性序、覆盖先于 limit、零通过告警、缺 qualifier 响亮失败、非正 limit 拒绝）；Task 10 T8 已裁决 KEEP（发现层规则，资格规则层权威无法替代） |
