...
| `tests/unit/test_cli_surface.py` | `test_cli_research_adapter.py`（池 9 例） | KEEP | 0 | 9 条各守适配器协议的一个面（提交 / 透传 / 状态 / 结果 / 未配置 / 失败 / 坏输出 / 缺字段等），断言结构各异，无同一断言的参数枚举 |
...
| `tests/unit/test_quality_gates.py` | `test_financial_quality_gate.py`（池 8 例） | KEEP | 0 | 不同上游状态与观察点（缺失≠缺陷、不可读单元使键无效、重复键同删、跨期非重复、空报表 P1、单坏点不阻断全市场、findings 带键等） |
...
| 2 | `a4130d2` | 文档：收口测试精简台账（Task 14 行与收口小节） | 本文件（§7.2 依据列、§8 Task 13 行回填、§8 Task 14 行、§10「等」、本节） |
| 3 | 本提交 | 文档：回填台账 Task 14 行提交哈希 | 本文件（仅回填两处 commit 单元格：§8 Task 14 行 + 本节提交表 #2） |
