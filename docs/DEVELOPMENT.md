...
- **快照绝对不可变（Immutability）**：同一日期的正式快照只要内容不同，存储层强制抛出 `SnapshotConflictError`。严禁在生产代码或测试辅助代码中增加任何自动迁移、重命名或删除 `data/snapshots/` 生产快照来绕过冲突的逻辑；历史回放与重估产物（如回溯测试、校准重估、方案验证等）一律仅限输出至 `var/acceptance/<run-name>/`，严禁通过擦除旧快照来伪造重跑成功。
